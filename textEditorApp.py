...
#Module means a single Python file. It has many functions inside it. There are many such useful things so that we don't have to write code again and again. We can use directly from there.     

#Library is a collection of many modules, sub modules, packages, many such things 

#Import tkinter module for creating GUI applications
#tkinder is both, a library and a module 
...
def open_file():
    ...
    if file_path:
        ...
        with open(file_path, "r") as file:        #if we use with keyword then we don't have to close the file, it closes automatically. 
